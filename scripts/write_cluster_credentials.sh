...
cat kubeconfig_$CLUSTER | opw write five-platform-${CLUSTER} kubeconfig -
...
terraform output cluster_endpoint | tr -d \\n | sed 's/"//g' | opw write five-platform-${CLUSTER} cluster-endpoint -
terraform output cluster_certificate_authority_data | tr -d \\n | sed 's/"//g' | opw write five-platform-${CLUSTER} base64-certificate-authority-data -
